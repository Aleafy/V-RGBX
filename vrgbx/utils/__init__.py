    ...
    def download_if_necessary(self, use_usp=False):
        ...
        if self.path is None:
            ...
            if not skip_download:
                ...
                search_path = os.path.join(self.local_model_path, self.model_id, self.origin_file_pattern)
                downloaded_files = glob.glob(search_path)

                print(f"Downloading {self.model_id} from {self.download_resource} to {self.local_model_path} with allow_file_pattern={allow_file_pattern} and skip_download={skip_download}.")
                snapshot_download(
                    self.model_id,
            ...
            if self.s3_download:
                fs = getattr(self, "_s3_fs", None) or s3fs.S3FileSystem(anon=False)
                self._s3_fs = fs

                s3_dir = f"s3://xxx/{self.model_id}"
                local_dir = os.path.join(self.local_model_path, self.model_id)
                os.makedirs(local_dir, exist_ok=True)

                # 列出所有文件（递归）
                files = [f for f in fs.find(s3_dir) if not f.endswith("/")]

                # 基于文件数量显示进度条
                for f in tqdm(files, desc=f"Downloading {self.model_id}", unit="file"):
                    rel = f.split(f"{self.model_id}/", 1)[-1]
                    local_f = os.path.join(local_dir, rel)
